...
gcloud run deploy bosque-api \
  ...
  --set-env-vars ALLOWED_DOMAIN=humboldt.org.co,DB_PASS=CAMBIAR,CORS_ORIGINS=*,AUDIENCE=TU_GOOGLE_CLIENT_ID,CLOUDSQL_CONNECTION_NAME=CONNECTION_NAME,DB_NAME=bosquebd,DB_USER=bosquebd
```

> **Importante (Autenticación):**
> - `AUDIENCE`: Debe ser el mismo Client ID de Google configurado en el frontend.
> - `BWhitelist`: Los usuarios deben estar registrados en la tabla `BWhitelist` de la base de datos con rol `admin` o `user`.
> - `CORS_ORIGINS`: Ajusta al dominio real del frontend en producción.
