...
def sync_table(table_erp, table_cloud, erp_engine, cloud_engine):
    ...
    try:
        ...
        # B. Leer códigos ya existentes en la Nube
        try:
            ...
            # Si la tabla no existe en la nube, empezamos de cero
            codes_in_cloud = set()
...
def run_sync():
    ...
    erp_url = f"mssql+pyodbc://{ERP_USER}:{safe_erp_pass}@{ERP_HOST}:{ERP_PORT}/{ERP_DB}?driver=ODBC+Driver+17+for+SQL+Server"
